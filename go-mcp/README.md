...
[MCP](https://modelcontextprotocol.io) server exposing the Moogleapi SDK as
two agent tools — `moogleapi_list` and `moogleapi_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/moogleapi-mcp)
make build

# 2. Provide credentials via the environment
export MOOGLEAPI_APIKEY=sk_live_xxx
...
```jsonc
// moogleapi_list: first page of records
{ "entity": "moogle_api_web_features_characters_get_all_get_all_character" }
{ "entity": "moogle_api_web_features_characters_get_all_get_all_character", "query": { } }

// moogleapi_load: one record by id
{ "entity": "moogle_api_web_features_characters_get_get_character", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/moogleapi-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user moogleapi \
     -- "$PWD"/dist/*/moogleapi-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `moogleapi_list` and `moogleapi_load` tools now appear
   in new sessions. Ask the agent to *"list moogle_api_web_features_characters_get_all_get_all_character using moogleapi"*
   and it calls `moogleapi_list` with `{"entity":"moogle_api_web_features_characters_get_all_get_all_character"}`.
...
```sh
export MOOGLEAPI_APIKEY=sk_live_xxx            # API key
export MOOGLEAPI_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `moogleapi_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `moogleapi_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `MOOGLEAPI_APIKEY` | API key sent with every request. |
| `MOOGLEAPI_BASE` | Optional override of the API base URL. |
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"moogleapi_load","arguments":{"entity":"moogle_api_web_features_characters_get_get_character","query":{"id":1}}}}'
```
