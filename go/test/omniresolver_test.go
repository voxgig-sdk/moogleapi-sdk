...
import (
	...
	omni "github.com/voxgig-sdk/moogleapi-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.MoogleapiError); is {
				out := map[string]any{
					"name":    "MoogleapiError",
...
func omniCtx(arg any, client *sdk.MoogleapiSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.MoogleapiSDK); is {
			client = live
