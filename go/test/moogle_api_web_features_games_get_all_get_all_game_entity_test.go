...
func TestMoogleApiWebFeaturesGamesGetAllGetAllGameEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.MoogleApiWebFeaturesGamesGetAllGetAllGame(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.MoogleApiWebFeaturesGamesGetAllGetAllGame(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		moogleApiWebFeaturesGamesGetAllGetAllGameRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.moogle_api_web_features_games_get_all_get_all_game")))
