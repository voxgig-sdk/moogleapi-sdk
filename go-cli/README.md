...
boru-driven command-line client **and** interactive REPL for the Moogleapi
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/moogleapi-cli)
make build
...
./moogleapi-cli list moogle_api_web_features_characters_get_get_character

# 5. Override the API base URL for a single call
MOOGLEAPI_BASE=https://api.example.com ./moogleapi-cli list moogle_api_web_features_characters_get_all_get_all_character

# 6. No arguments -> interactive REPL
./moogleapi-cli
moogleapi> list moogle_api_web_features_characters_get_all_get_all_character
moogleapi> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/moogleapi-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/moogleapi-cli list moogle_api_web_features_characters_get_all_get_all_character
   ```
...
Run with no arguments to open a REPL (prompt `moogleapi>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `moogle_api_web_features_characters_get_all_get_all_character`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/moogleapi-cli`. |
...
(`./moogleapi-cli <expr>`) and interactively (the REPL), and expressions compose the
