// Command Moogleapi-cli is a CLI + REPL for the Moogleapi SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      MOOGLEAPI_APIKEY (api key), MOOGLEAPI_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: MOOGLEAPI_APIKEY carries the API key and
	// MOOGLEAPI_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("MOOGLEAPI_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("MOOGLEAPI_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewMoogleapiSDK(opts)
