...
describe('MoogleApiWebFeaturesCharactersGetGetCharacterDirect', async () => {
  ...
  test('direct-load-moogle_api_web_features_characters_get_get_character', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-moogle_api_web_features_characters_get_get_character', setup.live)) return
