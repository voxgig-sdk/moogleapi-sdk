...
describe('MoogleApiWebFeaturesGamesGetGetGameDirect', async () => {
  ...
  test('direct-load-moogle_api_web_features_games_get_get_game', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-moogle_api_web_features_games_get_get_game', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['MOOGLEAPI_TEST_MOOGLE_API_WEB_FEATURES_GAMES_GET_GET_GAME_ENTID']
