...
describe('MoogleApiWebFeaturesMonstersSearchSearchMonsterDirect', async () => {
  ...
  test('direct-list-moogle_api_web_features_monsters_search_search_monster', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-moogle_api_web_features_monsters_search_search_monster', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['MOOGLEAPI_TEST_MOOGLE_API_WEB_FEATURES_MONSTERS_SEARCH_SEARCH_MONSTER_ENTID']
