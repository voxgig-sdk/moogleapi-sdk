...
describe('MoogleApiWebFeaturesMonstersGetGetMonsterDirect', async () => {
  ...
  test('direct-load-moogle_api_web_features_monsters_get_get_monster', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-moogle_api_web_features_monsters_get_get_monster', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['MOOGLEAPI_TEST_MOOGLE_API_WEB_FEATURES_MONSTERS_GET_GET_MONSTER_ENTID']
