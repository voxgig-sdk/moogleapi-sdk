...
describe('MoogleApiWebFeaturesGamesGetAllGetAllGameDirect', async () => {
  ...
  test('direct-list-moogle_api_web_features_games_get_all_get_all_game', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-moogle_api_web_features_games_get_all_get_all_game', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['MOOGLEAPI_TEST_MOOGLE_API_WEB_FEATURES_GAMES_GET_ALL_GET_ALL_GAME_ENTID']
