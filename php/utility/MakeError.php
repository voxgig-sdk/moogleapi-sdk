...
class MoogleapiMakeError
{
    public static function call(?MoogleapiContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new MoogleapiContext([], null);
        }
        $op = $ctx->op ?? new MoogleapiOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new MoogleapiResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof MoogleapiError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new MoogleapiError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof MoogleapiError) {
