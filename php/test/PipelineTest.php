...
{
    private static function utility(): MoogleapiUtility
    {
        return new MoogleapiUtility();
    }
    ...
    private static function ctx(array $over = []): MoogleapiContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new MoogleapiContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new MoogleapiOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof MoogleapiError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new MoogleapiOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = MoogleapiMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new MoogleapiOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = MoogleapiMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new MoogleapiOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = MoogleapiMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new MoogleapiSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = MoogleapiMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new MoogleapiResponse([]),
            'result' => new MoogleapiResult([]),
        ]);
        [, $err] = MoogleapiMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new MoogleapiSpec([]),
            'response' => null,
            'result' => new MoogleapiResult([]),
        ]);
        [, $err] = MoogleapiMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new MoogleapiSpec(['step' => 's']),
            'response' => new MoogleapiResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new MoogleapiResult([]),
        ]);
        [, $err] = MoogleapiMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new MoogleapiSpec(['step' => 's']),
            'response' => new MoogleapiResponse(self::resp(200, ['v' => 1])),
            'result' => new MoogleapiResult([]),
        ]);
        [, $err] = MoogleapiMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new MoogleapiSpec(['step' => 's']),
            'response' => new MoogleapiResponse(self::resp(200, ['v' => 2])),
            'result' => new MoogleapiResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        MoogleapiMakeResponse::call($ctx);
    ...
    {
        $preset = new MoogleapiResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new MoogleapiSpec([]),
            'response' => new MoogleapiResponse([]),
            'result' => new MoogleapiResult([]),
        ...
        [$got, $err] = MoogleapiMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new MoogleapiResult([])]);
        [, $err] = MoogleapiMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new MoogleapiSpec([]), 'result' => null]);
        [, $err] = MoogleapiMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new MoogleapiSpec(['step' => 's']),
            'result' => new MoogleapiResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new MoogleapiOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = MoogleapiMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new MoogleapiSpec(['step' => 's']),
            'result' => new MoogleapiResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new MoogleapiOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = MoogleapiMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new MoogleapiResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new MoogleapiSpec([]),
            'result' => new MoogleapiResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = MoogleapiMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = MoogleapiMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new MoogleapiError('boom', 'boom');
        $utility->fetcher = function (MoogleapiContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new MoogleapiSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = MoogleapiMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (MoogleapiContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = MoogleapiMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(MoogleapiResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (MoogleapiContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new MoogleapiSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = MoogleapiMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new MoogleapiResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new MoogleapiSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = MoogleapiMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = MoogleapiMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = MoogleapiMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new MoogleapiResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, MoogleapiDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new MoogleapiResult(['ok' => false])]);
        $this->expectException(MoogleapiError::class);
        MoogleapiDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new MoogleapiResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', MoogleapiMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        MoogleapiMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new MoogleapiBaseFeature();
        $b = new MoogleapiBaseFeature();
        MoogleapiFeatureAdd::call($ctx, $a);
        MoogleapiFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): MoogleapiBaseFeature
    {
        $f = new MoogleapiBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        MoogleapiFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): MoogleapiContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new MoogleapiSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = MoogleapiPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new MoogleapiResponse(['headers' => null]),
            'result' => new MoogleapiResult([]),
        ]);
        MoogleapiResultHeaders::call($ctx);
