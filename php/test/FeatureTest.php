...
class FtCtrl extends MoogleapiControl
{
...
class FtClient
{
    ...
    private MoogleapiUtility $utility;

    public function __construct(MoogleapiUtility $utility, array $options)
    {
    ...
    public function get_utility(): MoogleapiUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MoogleapiContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public MoogleapiContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (MoogleapiContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MoogleapiConfig::make_feature($name);
    ...
    private static function buildUrl(MoogleapiSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MoogleapiError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MoogleapiResponse($fetched);
        }
    ...
    private function fail(MoogleapiContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MoogleapiContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
