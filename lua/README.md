...
The Lua SDK for the Moogleapi API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:MoogleApiWebFeaturesCharactersGetAllGetAllCharacter(nil):list(nil, nil)
...
local result, err = client:Moogleapi(nil):load(
  { id = "test01" }, nil
)
