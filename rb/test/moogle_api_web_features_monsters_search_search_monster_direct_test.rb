...
class MoogleApiWebFeaturesMonstersSearchSearchMonsterDirectTest < Minitest::Test
  def test_direct_list_moogle_api_web_features_monsters_search_search_monster
    setup = moogle_api_web_features_monsters_search_search_monster_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-moogle_api_web_features_monsters_search_search_monster", setup[:live] ? "live" : "unit")
    if _should_skip
...
def moogle_api_web_features_monsters_search_search_monster_direct_setup(mockres)
