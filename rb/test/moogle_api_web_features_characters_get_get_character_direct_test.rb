...
class MoogleApiWebFeaturesCharactersGetGetCharacterDirectTest < Minitest::Test
  def test_direct_load_moogle_api_web_features_characters_get_get_character
    setup = moogle_api_web_features_characters_get_get_character_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-moogle_api_web_features_characters_get_get_character", setup[:live] ? "live" : "unit")
    if _should_skip
...
def moogle_api_web_features_characters_get_get_character_direct_setup(mockres)
