...
class MoogleapiSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = MoogleapiHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = MoogleapiHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, MoogleapiFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    MoogleapiUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    params = MoogleapiHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = MoogleapiHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = MoogleapiHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = MoogleapiError.new(
        "graphql_error", "MoogleapiSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
