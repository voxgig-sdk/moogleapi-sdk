...
class MoogleapiMakeRequest
{
    public static function call(MoogleapiContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new MoogleapiResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new MoogleapiResponse($fetched);
        } else {
