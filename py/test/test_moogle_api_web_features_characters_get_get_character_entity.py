...
class TestMoogleApiWebFeaturesCharactersGetGetCharacterEntity:

    def test_should_create_instance(self):
        testsdk = MoogleapiSDK.test(None, None)
        ent = testsdk.MoogleApiWebFeaturesCharactersGetGetCharacter(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _moogle_api_web_features_characters_get_get_character_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "moogle_api_web_features_characters_get_get_character." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set MOOGLEAPI_TEST_MOOGLE_API_WEB_FEATURES_CHARACTERS_GET_GET_CHARACTER_ENTID JSON to run live")
        client = setup["client"]
        ...
        moogle_api_web_features_characters_get_get_character_ref01_data = None
        if len(moogle_api_web_features_characters_get_get_character_ref01_data_raw) > 0:
        ...
        # LOAD
        moogle_api_web_features_characters_get_get_character_ref01_ent = client.MoogleApiWebFeaturesCharactersGetGetCharacter(None)
        ...
        moogle_api_web_features_characters_get_get_character_ref01_data_dt0_loaded, err = moogle_api_web_features_characters_get_get_character_ref01_ent.load(moogle_api_web_features_characters_get_get_character_ref01_match_dt0, None)
        assert err is None
...
def _moogle_api_web_features_characters_get_get_character_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/moogle_api_web_features_characters_get_get_character/MoogleApiWebFeaturesCharactersGetGetCharacterTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("MOOGLEAPI_TEST_MOOGLE_API_WEB_FEATURES_CHARACTERS_GET_GET_CHARACTER_ENTID"))
