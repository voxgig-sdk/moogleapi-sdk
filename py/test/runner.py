...
class MoogleapiTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            MoogleapiTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = MoogleapiTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = MoogleapiTestRunner.getenv("MOOGLEAPI_TEST_LIVE")
        override = MoogleapiTestRunner.getenv("MOOGLEAPI_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = MoogleapiTestRunner.getenv(key)
        ...
        explain = MoogleapiTestRunner.getenv("MOOGLEAPI_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["MOOGLEAPI_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if MoogleapiTestRunner._test_control is not None:
            return MoogleapiTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                MoogleapiTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            MoogleapiTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = MoogleapiTestRunner.load_test_control()
...
def load_env_local():
    MoogleapiTestRunner.load_env_local()


def env_override(m):
    return MoogleapiTestRunner.env_override(m)


def entity_data(v):
    return MoogleapiTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return MoogleapiTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return MoogleapiTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return MoogleapiTestRunner.load_test_control()


def live_client_options():
    return MoogleapiTestRunner.live_client_options()


def live_delay_ms():
    return MoogleapiTestRunner.live_delay_ms()
